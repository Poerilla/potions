...
| Asia-range London (USDJPY) | `live/fx_v2b_asia_range_london.py`, `…_usdjpy_sizing`, `…_usdjpy_filters`, `…_usdjpy_validation`, `…_usdjpy_filter_nulls` | Research/practice promote: filtered `S_3_1_3` — `FILTERS.md`; funded-sleeve gates: `VALIDATION_GATES.md`; nulls: `FILTER_NULLS.md` (risk throttle) |
...
3. **Funded-sleeve hardening** before calling it funded capital: frozen-rule OOS, walk-forward yearly/anchors, Jan/WR/PF attribution, path-aware risk logs, live-parity `campaign_parity.csv` vs research decision tape — driver `live/fx_v2b_asia_range_london_usdjpy_validation.py` → `VALIDATION_GATES.md`. Filter timing nulls (`…_filter_nulls`) decide alpha vs risk-throttle (`FILTER_NULLS.md`). Remember the **50-campaign** roll warmup (demos seed last-50).
