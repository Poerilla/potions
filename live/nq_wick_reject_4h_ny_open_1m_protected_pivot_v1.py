"""NQ 4h WICK_REJECT → NY-open 1m protected-pivot study (V1).

Descriptive only: no trades, fills, stops, targets, P&L, plugin, or S1/S2 coupling.

Hub: live/state/nq_wick_reject_4h_ny_open_1m_protected_pivot_v1/

Usage:
  export PYTHONPATH="/home/tester/hsm:/home/tester/hsm/potions/v20-python/src"
  python -m live.nq_wick_reject_4h_ny_open_1m_protected_pivot_v1 --smoke --email
  python -m live.nq_wick_reject_4h_ny_open_1m_protected_pivot_v1 --email
"""
...
HUB = REPO / "live" / "state" / "nq_wick_reject_4h_ny_open_1m_protected_pivot_v1"
STUDY_ID = "nq_wick_reject_4h_ny_open_1m_protected_pivot_v1"
NY = "America/New_York"
NY_OPEN = time(9, 30)
...
def _complete_1m_open_to_obs(day: pd.DataFrame) -> bool:
    """Require near-complete 1m coverage 09:30–13:00 ET."""
    if day is None or day.empty:
        return False
...
def build_strict_1m_pivots(
    ...
    if day_1m is None or len(day_1m) < 3:
        return []
    hi = day_1m["high"].to_numpy(dtype=float)
    lo = day_1m["low"].to_numpy(dtype=float)
    idx = day_1m.index
    pivots: List[Dict[str, Any]] = []
    open_ts = _localize(open_ts)
    pid = 0
    for i in range(1, len(day_1m) - 1):
        ...
        if not is_h and not is_l:
            continue
        pivot_ts = _localize(idx[i])
        avail = _localize(idx[i + 1])
        ptype = "H" if is_h else "L"
        price = float(hi[i] if is_h else lo[i])
        pid += 1
        bar_i = int(round((pivot_ts - open_ts).total_seconds() / 60.0))
        inside = (pivot_ts.time() >= NY_OPEN) and (avail.time() <= FORMATION_CUTOFF) and (
            pivot_ts.date() == open_ts.date()
        )
        # also allow avail exactly at cutoff via timestamp compare
        inside = (_localize(pivot_ts) >= open_ts) and (avail <= _ts_at(open_ts.date(), FORMATION_CUTOFF))
        pivots.append(
            {
                "seed_id": seed_id,
                "ny_date": ny_date,
...
def select_structure(
    ...
    window = [
        p
        for p in pivots
        if p["_pivot_ts"] >= open_ts and p["_avail"] <= cutoff
    ]
    if len(window) < 4:
    ...
    for i in range(len(window) - 3):
        ...
        if d["_avail"] > cutoff:
            continue
        if d["_avail"] >= _ts_at(open_ts.date(), OBS_END):
            continue
        ...
        row = {
            ...
            "1m_direction_vs_seed_direction": _dir_vs_seed(pattern, str(elig["seed_direction"])),
    ...
    # Check cutoff failures: type-valid but P4 after cutoff
    for i in range(len(pivots) - 3):
        a, b, c, d = pivots[i], pivots[i + 1], pivots[i + 2], pivots[i + 3]
        if a["_pivot_ts"] < open_ts:
            continue
        if _is_bear_tuple(a, b, c, d) or _is_bull_tuple(a, b, c, d):
            if d["_avail"] > cutoff:
...
def evaluate_protection(
    ...
    bars = day_1m[(day_1m.index > complete) & (day_1m.index < obs_end)]
    expected_minutes = max(0, int((obs_end - complete).total_seconds() // 60) - 0)
    # rough completeness: need most bars after completion
    data_complete = True
    if expected_minutes >= 5 and len(bars) < max(3, int(0.85 * expected_minutes)):
        data_complete = False
    ...
    for ts, row in bars.iterrows():
        ...
        lo = float(row["low"])
        if pattern == "BEAR":
            # favorable = downside; adverse = upside vs protected HH
    ...
    def _held_until(t_end: time) -> bool:
        end = _ts_at(d, t_end)
    ...
    # held_to checkpoints use strict break definition
    ...
    out["held_to_1030"] = not _broken_before(FORMATION_CUTOFF)
...
def run_causality_audit(
    ...
    for _, c in cands.iterrows():
        ...
        if not (ny_open <= p1ts):
            fails.append("%s ny_open > p1" % c["candidate_id"])
    ...
    n_cand = len(cands)
    n_out = len(outs)
    n_insuff = int((outs["outcome_label"] == "INSUFFICIENT_DATA").sum()) if len(outs) else 0
    if n_cand != n_out:
...
def compute_summary(
    ...
    worth = bool(sample_ok and rates_ok and excursion_ok and causality_ok)
    if not sample_ok:
        stance = "DESCRIPTIVE ONLY / UNDER-SAMPLED — NOT WORTH FURTHER RESEARCH CLAIM"
    elif worth:
...
def generate_charts(
    ...
    from . import nq_wick_reject_4h_ny_open_1m_protected_pivot_v1_charts as charts
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    rid = begin_run(
        ...
        meta={"descriptive_only": True, "config_hash": cfg_hash, "smoke": args.smoke},
    )
    ...
    try:
        ...
        for i, (_, er) in enumerate(included.iterrows()):
            ...
            day = _day_rth(gby, d)
            pivots = build_strict_1m_pivots(day, er["seed_id"], er["ny_date"], open_ts)
            for p in pivots:
            ...
            if cand is None:
                ...
                out = evaluate_protection(cand, day)
        ...
        ok, fails = run_causality_audit(elig, cands, outs)
