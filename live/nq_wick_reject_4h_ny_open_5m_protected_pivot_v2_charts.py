"""ECU chart packs A–D for nq_wick_reject_4h_ny_open_5m_protected_pivot_v2."""
...
OBS_END = time(13, 0)
_ACTIVE_FORMATION_CUTOFF = FORMATION_CUTOFF
_NO_CUTOFF = False
...
def _pivot_rule_lines() -> List[str]:
    return [
        "Pivot rule: 1-left / 1-right strict 5-minute pivots",
        "Candidate bar timeframe: 5 minutes",
        "Causal confirmation: pivot available only after subsequent completed 5m bar",
    ]
...
def _session_guides(ax, d: date, shade_formation: bool = True) -> None:
    o = _ts_at(d, NY_OPEN)
    c = _ts_at(d, _ACTIVE_FORMATION_CUTOFF)
    e = _ts_at(d, OBS_END)
...
def render_all(
    ...
    smoke_cap: int,
    tick: float,
    formation_cutoff: Optional[time] = None,
    no_cutoff: bool = False,
) -> None:
    global _ACTIVE_FORMATION_CUTOFF, _NO_CUTOFF
    _ACTIVE_FORMATION_CUTOFF = formation_cutoff or FORMATION_CUTOFF
    _NO_CUTOFF = bool(no_cutoff)
...
def _chart_pack_a(path, er, cand, out_by_id, pivots_df, gby, h4, study_id, data_version, cfg_hash, gen_ts, tick):
    ...
    day = _day_rth(gby, d)
    day5 = _to_5m(day)
    inset = day5[(day5.index >= _ts_at(d, time(9, 20))) & (day5.index < _ts_at(d, time(10, 40)))]
    out_panel = day5[(day5.index >= _ts_at(d, NY_OPEN)) & (day5.index < _ts_at(d, OBS_END))]
    ...
    _header(
        ...
        [
            ...
            "outcome=%s | %s" % (outcome or "(no candidate)", _pivot_rule_lines()[0]),
        ],
    )
    ...
    _plot_candles(ax1, inset, width_min=5)
    _session_guides(ax1, d)
    _mark_pivots(ax1, pivots_df, er["seed_id"], er["ny_date"], cand)
    ax1.set_ylabel("5m 09:20–10:40")

    _plot_candles(ax2, out_panel, width_min=5)
...
def _chart_pack_b(path, er, cand, out, day, study_id, data_version, cfg_hash, gen_ts, tick):
    ...
    main = day5[(day5.index >= _ts_at(d, time(9, 20))) & (day5.index <= end)]
    ext = day5[(day5.index >= _ts_at(d, NY_OPEN)) & (day5.index < _ts_at(d, OBS_END))]
    ...
    _header(
        ...
        [
            ...
            "pivot=1L/1R strict 5m | hash=%s | %s" % (cfg_hash, data_version),
            "seed_dir=%s | pattern=%s | ctx=%s | vs_seed=%s | age_h=%.1f"
            % (
                er["seed_direction"],
                cand["pattern"],
                cand["seed_context_relation"],
                cand["5m_direction_vs_seed_direction"],
                float(cand["seed_age_hours"]),
            ),
            "gen=%s | break_dist=%.1f ticks | dur=%.0f min | %s"
            % (
                gen_ts,
                float(cand["break_distance_ticks"]),
                float(cand["sequence_duration_minutes"]),
                _pivot_rule_lines()[2],
            ),
        ],
    )
    _plot_candles(ax0, main, width_min=5)
...
def _chart_pack_c(path, er, cand, out, day, study_id, data_version, cfg_hash, gen_ts, tick):
    ...
    day5 = _to_5m(day)
    complete = _localize(pd.Timestamp(cand["structure_complete_at"]))
    panel = day5[(day5.index >= complete) & (day5.index < _ts_at(d, OBS_END))]
    ...
    _header(
        ...
        [
            ...
            "hash=%s | %s | gen=%s | obs=5m" % (cfg_hash, data_version, gen_ts),
        ],
    )
    _plot_candles(ax, panel, width_min=5)
...
def _chart_pack_d(path, er, meta, pivots_df, gby, study_id, data_version, cfg_hash, gen_ts, tick):
    ...
    day = _day_rth(gby, d)
    day5 = _to_5m(day)
    panel_end = time(13, 5) if _NO_CUTOFF else time(10, 45)
    panel = day5[(day5.index >= _ts_at(d, time(9, 20))) & (day5.index < _ts_at(d, panel_end))]
    fig, ax = plt.subplots(1, 1, figsize=(12, 5.5))
    ...
    _header(
        ...
        [
            ...
            "hash=%s | %s | gen=%s | %s" % (cfg_hash, data_version, gen_ts, _pivot_rule_lines()[0]),
        ],
    )
    _plot_candles(ax, panel, width_min=5)
