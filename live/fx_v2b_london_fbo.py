"""London v2b **FBO** on the classic **15m** London OR (03:00–03:15).

Uses ``entry_mode=first_break_opposite`` (ignore first OR break → arm opposite
stop; no reverse leg; ``max_campaigns=1``). Broker-like Engine+PaperBroker.

Note: the 4h OR FBO majors book already banked negative under
``live/state/fx_v2b_london_fbo/`` (kept). This run writes
``live/state/fx_v2b_london_fbo_15m/``.

Default markets: FX majors (EURUSD / GBPUSD / USDJPY). Books: S_1_1_3, S_1_1_1.
"""
...
DEFAULT_OUT = REPO / "live" / "state" / "fx_v2b_london_fbo_15m"
CLOCK = CLOCKS["london_open"]
OR_BARS = 15
...
def run_one(
    *,
    ...
    strategy_id = "%s_v2b_london_fbo_15m_%s" % (market.symbol.lower(), book)
    ...
    write_run_manifest(
        ...
        extra={"driver": "fx_v2b_london_fbo", "book": book, "or_minutes": 15},
    )
    _progress(
        output_root,
...
def write_summary(output_root: Path, rows: List[dict]) -> None:
    ...
    lines = [
        "# FX majors v2b — London **15m OR FBO** (03:00–03:15)",
        "",
        "StrategyPlugin `v2b_scaleout`, `entry_mode=first_break_opposite`, `or_bars=15`, "
        "arm after 03:15, flatten 11:59, **max_campaigns=1** (no reverse leg).",
        "",
    ...
    email = [
        "potions: fx_v2b_london_fbo_15m complete",
        "",
        "London 15m OR FBO 03:00–03:15 NY → ignore first break → arm opposite; flatten 11:59; max_campaigns=1.",
        "Broker-like Engine+PaperBroker. Compare vs OCO 15m London ungated + 4h FBO hubs.",
        "",
        "Top by N/S:",
    ]
    for r in ranked[:12]:
...
def run_batch(
    *,
    ...
    _progress(output_root, "START london_fbo_15m markets=%s books=%s" % (",".join(markets), ",".join(books)))
    errors: List[str] = []
    for name in markets:
        key = name.upper()
        market = MARKETS[key]
        ...
        pending = [
            ...
            or (
                (key, b) not in seen
                and not (
                    output_root / "states" / ("%s_v2b_london_fbo_15m_%s" % (key.lower(), b)) / "metrics.json"
                ).exists()
            )
        ]
        if not pending:
            for book in books:
                mp = output_root / "states" / ("%s_v2b_london_fbo_15m_%s" % (key.lower(), book)) / "metrics.json"
    ...
    if email:
        try:
            ...
            if errors:
                body += "\n\nErrors:\n" + "\n".join(errors)
            send_email(subject="potions: fx_v2b_london_fbo_15m complete", body=body)
            ...
            try:
                ...
                send_email(
                    subject="potions: fx_v2b_london_fbo_15m EMAIL/ partial",
                    body="Hub: %s\nEmail body write/send issue: %s\nErrors: %s"
