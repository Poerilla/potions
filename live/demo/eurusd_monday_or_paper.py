"""EURUSD Monday OR Phase 2 primary — paper demo (OANDA prices, PaperBroker fills).

Tracker / Phase 2 default: ``M1_S2_R2`` (``monday_or_breakout``), N/S 1.74 paper-half + Aug skip.
Artifacts: ``live/demo/eurusd_monday_or_m1_s2_r2_half_paper/``.
...
INSTRUMENT = "EURUSD"
STRATEGY_ID = "eurusd_monday_or_m1_s2_r2_half_paper"
RUN_DIRNAME = "eurusd_monday_or_m1_s2_r2_half_paper"
PHASE2_TAG = PAIR_PHASE2_DEFAULT["EURUSD"]  # M1_S2_R2
TICK = 0.00001
CLI_COMMAND = "demo-eurusd-monday-or-paper"
...
def strategy_config_payload() -> Dict[str, Any]:
    payload = plugin_config(TICK, PHASE2_TAG, pair="EURUSD")
    # Paper-only half-size (Phase 2 sub-period FAIL) + Aug skip from filter screen.
    payload.update(
        {
            "entry_qty": 2,
            "dd30_qty": 1,
...
def write_run_meta(output_root: Path, *, config: OandaConfig) -> Dict[str, Any]:
    ...
    meta = {
        ...
        "tracker": "STRATEGY_TRACKER Monday OR FX + monday_or_sizing_sweep_broker EURUSD #1 M1_S2_R2 N/S 8.20",
...
def run_stream_loop(
    *,
    ...
    append_progress(
        output_root,
        "STARTED EURUSD Monday OR paper tag=%s strategy=%s account=%s state=%s pid=%s"
