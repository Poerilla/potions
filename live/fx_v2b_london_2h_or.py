"""London v2b with a **2-hour** opening range (03:00–05:00 NY).

Same StrategyPlugin ``v2b_scaleout`` / broker realism as the 15m London book, but:
  - OR watch: 03:00–05:00 (``or_bars=120``)
  - Arm OCO after 05:00 using the 2h high/low
  - Flatten 11:59

Default books: S_1_1_3 and S_1_1_1 for comparison to the 15m London ungated hub.
"""
...
DEFAULT_OUT = REPO / "live" / "state" / "fx_v2b_london_2h_or"
CLOCK = CLOCKS["london_2h_or"]
OR_BARS = 120
...
def run_one(
    *,
    ...
    strategy_id = "%s_v2b_london_2h_or_%s" % (market.symbol.lower(), book)
    ...
    write_run_manifest(
        ...
        extra={"driver": "fx_v2b_london_2h_or", "book": book},
    )
    _progress(
        output_root,
...
def write_summary(output_root: Path, rows: List[dict]) -> None:
    ...
    email = [
        "potions: fx_v2b_london_2h_or complete",
        "",
        "London 2h OR 03:00–05:00 NY → arm after 05:00 → flatten 11:59.",
        "Compare vs 15m London ungated hub live/state/fx_v2b_london_ungated/",
        "",
        "Top by N/S:",
    ]
    for r in ranked[:12]:
...
def run_batch(
    *,
    ...
    _progress(output_root, "START london_2h_or markets=%s books=%s" % (",".join(markets), ",".join(books)))
    errors: List[str] = []
    for name in markets:
        key = name.upper()
        market = MARKETS[key]
        ...
        pending = [
            ...
            or (
                (key, b) not in seen
                and not (output_root / "states" / ("%s_v2b_london_2h_or_%s" % (key.lower(), b)) / "metrics.json").exists()
            )
        ]
        if not pending:
            for book in books:
                mp = output_root / "states" / ("%s_v2b_london_2h_or_%s" % (key.lower(), book)) / "metrics.json"
    ...
    if email:
        try:
            ...
            if errors:
                body += "\n\nErrors:\n" + "\n".join(errors)
            send_email(subject="potions: fx_v2b_london_2h_or complete", body=body)
