"""NAS100 prior-opposed broker-like replay: hourly ST+PMC gate → v2b.

Uses MT5 NAS100 1m (``fx/raw/NAS100_1m_data.csv``) as an NQ-style index CFD
proxy. ST stops/targets are **index points** (25/75), matching the NQ Databento
prior-opposed path — not FX pips.

Economics note: ``POINT_VALUE=1.0`` ($1/pt/unit) vs NQ ``20.0``. Compare trade
shape (counts, win%, PF, N/S) directly; scale dollar nets ×20 for a rough
NQ-contract-equivalent reading.
"""
...
# 25/75 index points — same absolute stop distance as NQ ST+PMC convention.
ST_VARIANT = VariantConfig(
    "sl25_tp75_3r",
    stop_pts=25.0,
    tp1_pts=75.0,
    notes="NAS100 25/75 index-point ST+PMC (NQ-style).",
)
ST_STRATEGY_ID = "%s_hourly_st_pmc_%s" % (MARKET, ST_VARIANT.name)
V2B_STRATEGY_ID = "%s_v2b_prior_opposed_stpmc_only_S_1_1_3" % MARKET
POINT_VALUE = 1.0
FEE_PER_UNIT = 1.50  # match NQ broker-like fee per closed unit
DEFAULT_START = date(2021, 3, 4)  # NQ Databento prior-opposed start
...
def load_nas100_hourly_bars(one_m: Path) -> List[Bar]:
...
def run_st_pmc(
    *,
    ...
    print("Loading NAS100 hourly bars for ST+PMC...", flush=True)
    bars = load_nas100_hourly_bars(one_m)
    if max_bars:
        bars = bars[:max_bars]
...
def run_v2b_prior_opposed(
    *,
    ...
    print("Loading NAS100 1m bars for v2b prior-opposed...", flush=True)
    gby = load_fx_1m_by_ny_date(one_m, INSTRUMENT)
    st_orders = st_fills.parent / "orders.csv"
    if st_orders.exists():
        print("Refining NAS100 ST+PMC gate timestamps with 1m first-touch...", flush=True)
    ...
    for idx, day in enumerate(regime_dates, start=1):
        ...
        if idx % 250 == 0:
            print("  NAS100 %d/%d sessions" % (idx, len(regime_dates)), flush=True)
    store.flush_tables()
    ...
    write_run_manifest(
        ...
        broker_realism_config={
            ...
            "spread_model": "default",
        },
        causality_mode="audit",
        extra={
            "driver": "nas100_prior_opposed_replay",
            "causality_violations": result.causality_violations,
            "timezone_note": "MT5 DateTime localized as Europe/Athens then converted to UTC/NY",
            "nq_point_value_scale": 20.0,
            "net_usd_nq_equivalent": result.net_usd * 20.0,
        },
    )
    return result
...
def run(
    *,
    ...
    one_m, daily = ensure_nas100_platform_files(REPO, force=force_convert)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="NAS100 ST+PMC → v2b prior-opposed StrategyPlugin broker-like replay."
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=REPO / "live" / "state" / "nas100_v2b_prior_opposed_stpmc_broker_like",
    )
    ...
    parser.add_argument("--force-convert", action="store_true", help="Rebuild fx/nas100_*.csv from MT5 raw.")
    ...
    print(
        "Wrote %s (Net=$%.2f Net/Stress=%.2f causality_violations=%d; NQ-equiv Net~$%.2f)"
        % (
