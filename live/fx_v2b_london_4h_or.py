"""London v2b with a **4-hour** opening range (03:00–07:00 NY).

Same StrategyPlugin ``v2b_scaleout`` / broker realism as the 15m London book, but:
  - OR watch: 03:00–07:00 (``or_bars=240``; bars 03:00–06:59)
  - Arm OCO after 07:00 using the 4h high/low
  - Flatten 11:59
  - ``max_campaigns=2``: one re-entry allowed on the same OR after the first
    OCO campaign (entry + optional reverse) finishes

Default books: S_1_1_3 and S_1_1_1 for comparison to the 15m / 2h London hubs.
"""
...
DEFAULT_OUT = REPO / "live" / "state" / "fx_v2b_london_4h_or"
CLOCK = CLOCKS["london_4h_or"]
OR_BARS = 240


def _has_2h_session(raw_day: Optional[pd.DataFrame], session: date) -> bool:
    # Reuse densify window quality: need enough bars in 03:00–12:00 and OR coverage.
    from .fx_v2b_london_ungated import LONDON as _  # noqa: F401

    raw = session_bars(raw_day, session, CLOCK, dense=False)
    if raw.empty or len(raw) < max(80, CLOCK.min_session_bars // 3):
        return False
    or_slice = raw[raw.index.map(lambda ts: ts.time() < CLOCK.or_end)]
    return len(or_slice) >= CLOCK.min_or_bars
...
def run_one(
    *,
    ...
    strategy_id = "%s_v2b_london_4h_or_%s" % (market.symbol.lower(), book)
    ...
    if regime_dates is None:
        ...
        regime_dates = [d for d in regime_dates if _has_2h_session(gby.get(d), d)]
    ...
    for idx, day in enumerate(regime_dates, start=1):
        if session_frames is not None:
            ...
            df = session_bars(gby.get(day), day, CLOCK, dense=True)
    ...
    write_run_manifest(
        ...
        extra={"driver": "fx_v2b_london_4h_or", "book": book},
    )
    _progress(
        output_root,
...
def write_summary(output_root: Path, rows: List[dict]) -> None:
    ...
    email = [
        "potions: fx_v2b_london_4h_or complete",
        "",
        "London 4h OR 03:00–07:00 NY → arm after 07:00 → flatten 11:59; max_campaigns=2 (re-entry).",
        "Compare vs 15m / 2h London hubs.",
        "",
        "Top by N/S:",
    ]
    for r in ranked[:12]:
...
def run_batch(
    *,
    ...
    _progress(output_root, "START london_4h_or markets=%s books=%s" % (",".join(markets), ",".join(books)))
    errors: List[str] = []
    for name in markets:
        key = name.upper()
        market = MARKETS[key]
        ...
        pending = [
            ...
            or (
                (key, b) not in seen
                and not (output_root / "states" / ("%s_v2b_london_4h_or_%s" % (key.lower(), b)) / "metrics.json").exists()
            )
        ]
        if not pending:
            for book in books:
                mp = output_root / "states" / ("%s_v2b_london_4h_or_%s" % (key.lower(), book)) / "metrics.json"
        ...
        try:
            ...
            regime_dates = [d for d in _regime_dates(daily, gby, eff_start) if _has_2h_session(gby.get(d), d)]
            if max_days is not None:
                regime_dates = regime_dates[:max_days]
            session_frames: Dict[date, pd.DataFrame] = {
                day: session_bars(gby.get(day), day, CLOCK, dense=True) for day in regime_dates
            }
            _progress(output_root, "  %s sessions=%d" % (key, len(regime_dates)))
    ...
    if email:
        try:
            ...
            if errors:
                body += "\n\nErrors:\n" + "\n".join(errors)
            send_email(subject="potions: fx_v2b_london_4h_or complete", body=body)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--markets", default=",".join(MARKETS.keys()))
    p.add_argument("--books", default="S_1_1_3,S_1_1_1")
