"""HA (high-probability conditions) mill for USDJPY monthly ORB FBO 1/1/3 atr80.

Source book: ``fx_cross_pair_tracker_leaders/states/fbo_1_1_3_atr80_usdjpy``
(~+$108k / −$25k / N/S 4.25 / 156 campaigns, Engine+PaperBroker).

Pipeline (same machinery as intraday / quarterly / liq-run HA):

1. Condition profile on broker campaign tape (entry-asof features)
...
Usage::
  ...
  python -m live.usdjpy_monthly_orb_fbo_ha_conditions --email
  python -m live.usdjpy_monthly_orb_fbo_ha_conditions --email --smoke
  python -m live.usdjpy_monthly_orb_fbo_ha_conditions --email --profile-only
"""
...
HUB = REPO / "live" / "state" / "usdjpy_monthly_orb_fbo_ha_conditions"
PROFILE_HUB = HUB / "profile"
OVERLAY_HUB = HUB / "overlay"
NULLS_HUB = HUB / "nulls"
NY = "America/New_York"
MIN_N_DEFAULT = 12  # 156 campaigns — thin vs intraday, above quarterly floor
SOURCE_FILLS = (
    REPO
    / "live/state/fx_cross_pair_tracker_leaders/states/fbo_1_1_3_atr80_usdjpy/fills.csv"
)
BASELINE_NOTE = "USDJPY FBO 1/1/3 atr80 broker (~+$108k / N/S 4.25 / 156)"
...
BOOK = FboBook(
    ...
    "USDJPY Monthly ORB FBO 1/1/3 atr80 (broker)",
    "USDJPY",
    SOURCE_FILLS,
)
...
def load_campaigns(book: FboBook) -> pd.DataFrame:
...
def run_profile(*, min_n: int, email: bool = False) -> pd.DataFrame:
    ...
    # Attach N/S for ledger (campaign-sum stress is not MTM; leave stress blank).
    ...
    lines = [
        ...
        "",
        "min_n=%d. Nets USD-normalized (JPY/110)." % min_n,
        "",
        "## Book",
        "",
    ...
    email_body = (
        "potions: USDJPY FBO HA profile complete\n\n"
    ...
    if email:
        send_email(subject="potions: USDJPY FBO HA profile complete", body=email_body)
...
def run_overlay(*, email: bool = False, min_n: int = MIN_N_DEFAULT) -> pd.DataFrame:
    ...
    email_body = [
        "potions: USDJPY FBO HA overlay complete",
        "",
        "Hub: %s" % OVERLAY_HUB,
    ...
    if email:
        send_email(subject="potions: USDJPY FBO HA overlay complete", body=email_txt)
...
def run_nulls(*, email: bool = False, smoke: bool = False) -> pd.DataFrame:
    ...
    if not pairs:
        ...
        (NULLS_HUB / "EMAIL.txt").write_text(
            "potions: USDJPY FBO HA nulls skipped\n\nHub: %s\n%s\n" % (NULLS_HUB, msg),
            encoding="utf-8",
        )
        if email:
            send_email(
                subject="potions: USDJPY FBO HA nulls skipped",
    ...
    email_body = "potions: USDJPY FBO HA nulls complete\n\nHub: %s\nPairs: %d\n\n" % (
        NULLS_HUB,
        len(results),
    )
    for r in results:
    ...
    if email:
        send_email(subject="potions: USDJPY FBO HA nulls complete", body=email_body)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        if args.profile_only:
            ...
            if args.email:
                send_email(
                    subject="potions: USDJPY FBO HA profile-only complete",
            ...
            complete_run(
                ...
                meta={"source": "fbo_1_1_3_atr80_usdjpy", "pipeline": "profile"},
        ...
        body_parts = [
            "potions: USDJPY FBO HA mill complete",
            "",
            "Hub: %s" % HUB.resolve(),
            "Book: %s" % BASELINE_NOTE,
            "Stance: diagnostic HA — promote size-up only if nulls VALIDATED.",
            "",
        ]
        for label, p in (
        ...
        (HUB / "RUN_COMPLETE.json").write_text(
            json.dumps(
                {"ok": True, "hub": str(HUB), "decisions": decisions},
                indent=2,
            )
            + "\n",
            encoding="utf-8",
        ...
        if args.email:
            send_email(subject="potions: USDJPY FBO HA mill complete", body=body)
        ...
        (HUB / "EMAIL.txt").write_text(
            "potions: USDJPY FBO HA mill FAILED\n\nHub: %s\n\n%s\n" % (HUB, err),
            encoding="utf-8",
        )
        fail_run(rid, notes="HA mill crashed", meta={"error": err[-2000:]})
        if args.email:
            send_email(
                subject="potions: USDJPY FBO HA mill FAILED",
