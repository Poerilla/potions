#!/usr/bin/env python3
"""
Random case studies for swept-liquidity ORB breakout backtest
(`swept_liquidity_orb_breakout/sweep_orb_backtest.py`).

Input: ``swept_liquidity_orb_breakout/mnq_swept_orb_breakout.csv`` (2 MNQ, +1R/+2R, SL @ 1R).

Output: ``swept_liquidity_orb_breakout/charts/*.png`` + INDEX.md (unless --dates only).
"""
...
OUT_DIR = ROOT / 'swept_liquidity_orb_breakout' / 'charts'
TICK = 0.25
...
def draw_chart(date_obj, df1, csv_rows, outpath):
    ...
    # Prefer RH/RL from CSV if present (matches backtest)
    if pd.notna(csv_rows.iloc[0].get('RH')):
        rh = float(csv_rows.iloc[0]['RH'])
    if pd.notna(csv_rows.iloc[0].get('RL')):
        rl = float(csv_rows.iloc[0]['RL'])
    if pd.notna(csv_rows.iloc[0].get('Range')):
        rv = float(csv_rows.iloc[0]['Range'])

    filled = csv_rows[pd.notna(csv_rows['Entry_Price'])]
    skips = csv_rows[pd.isna(csv_rows['Entry_Price'])]
    ...
    for _, r in filled.iterrows():
        ...
        if str(r['Result']).startswith('EOD'):
            x_time = df1.index[-1]
        net_usd = float(r['Net_$']) if pd.notna(r['Net_$']) else 0.0
        pl_pts = float(r['Trade_PL_pts']) if pd.notna(r['Trade_PL_pts']) else 0.0
        tp1 = float(r['TP1_Level']) if pd.notna(r['TP1_Level']) else (rh + rv if d == 'Long' else rl - rv)
        tp2 = float(r['TP2_Level']) if pd.notna(r['TP2_Level']) else tp1
        sl = float(r['SL_Level']) if pd.notna(r['SL_Level']) else (rl if d == 'Long' else rh)
        sk_lvl = float(r['Skip_Level_or_ref']) if pd.notna(r.get('Skip_Level_or_ref')) else None
        trades.append(
            {
                'direction': d,
                'brk1': r['brk1_dir'],
    ...
    pattern = '+'.join([f"{t['direction'][0]}{str(t['result'])[0]}" for t in trades]) if trades else '—'
    sum_idx = sum(t['pl_pts'] for t in trades)
    day_net_usd = sum(t['net_usd'] for t in trades)
    sym = csv_rows['Symbol'].iloc[0]
    skip_note = ''
    if len(skips) > 0:
        reasons = skips['Result'].astype(str).value_counts().to_dict()
        skip_note = ' · skips: ' + ', '.join(f'{k}:{v}' for k, v in reasons.items())
    ...
    # Classic full OR targets (brk1 reference)
    ax.axhline(rh + rv, color='#64B5F6', linestyle=(0, (2, 4)), linewidth=0.9, alpha=0.75, zorder=2)
    ax.axhline(rl - rv, color='#64B5F6', linestyle=(0, (2, 4)), linewidth=0.9, alpha=0.75, zorder=2)
    ...
    for i, t in enumerate(trades, 1):
        ...
        ax.annotate(
            f"#{i} {t['direction'][0]} b1={str(t['brk1'])[0]} entry @{t['entry']:.2f}",
            xy=(x_e, t['entry']),
        ...
        ax.plot([x_e, x_x], [t['tp1'], t['tp1']], color='#81C784', linewidth=1.0, linestyle='-', alpha=0.65, zorder=4)
        ax.plot([x_e, x_x], [t['tp2'], t['tp2']], color='#66BB6A', linewidth=1.0, linestyle='-', alpha=0.55, zorder=4)
        ...
        ax.annotate(
            f"#{i} {t['result']} {t['pl_pts']:+.1f}pt (${t['net_usd']:+,.0f} net 2µ)",
            xy=(x_x, t['exit_price']),
            xytext=(8, -label_offset[(i - 1) % len(label_offset)]),
            textcoords='offset points',
            color=c,
    ...
    ax.text(last_x, rh + rv, f' +R {rh + rv:.2f}', color='#64B5F6', fontsize=7, va='center')
    ax.text(last_x, rl - rv, f' −R {rl - rv:.2f}', color='#64B5F6', fontsize=7, va='center')

    title = f'{date_obj}  ·  {sym}  ·  Swept ORB breakout  ·  Range {rv:.1f} pts  ·  Pattern {pattern}'
    subtitle = f'2× MNQ · Day net Σ ${day_net_usd:+,.2f} · Σ idx-pt {sum_idx:+,.2f}{skip_note}'
...
def main():
    ...
    ap.add_argument(
        ...
        help='Path to mnq_swept_orb_breakout.csv',
    )
    args = ap.parse_args()

    csv_path = Path(args.csv)
    if not csv_path.is_file():
        raise SystemExit(f'Missing CSV: {csv_path}\nRun sweep_orb_backtest.py first.')
    ...
    # Days with at least one row (trade or skip)
    ...
    with open(idx, 'w') as f:
        f.write('# Swept-liquidity ORB breakout — random samples (2× MNQ)\n\n')
        f.write(
            'First 5m break → pullback inside range → opposite 5m break → limit @ brk2 close; '
            'TP1 +1R / TP2 +2R; SL @ −1R; skip if brk1 classic target hits before fill.\n\n'
        )
        f.write(f'Sampled from `{csv_path}` (seed={args.seed}, start>={args.start}).\n\n')
        cats = Counter()
        for r in rows:
            cats[r['pattern']] += 1
        ...
        f.write(
            f'**Summary:** {len(rows)} charts, {wins} green days ({wins / max(len(rows), 1) * 100:.1f}%), Σ net ${total:,.0f}.\n\n'
        )
