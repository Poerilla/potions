#!/usr/bin/env python3
"""
Build ~50 PNG charts (25 winners, 25 losers) for **London sweep breaker** trades,
spread across calendar months, into ``case_studies/temp/``.

Reads ``data/mnq_v2e_london_sweep_breaker.csv`` (regenerate via ``backtest_london_sweep_breaker.py``).

Before writing, **clears** prior ``*.png`` and ``INDEX.md`` in ``--out`` so leftover filenames from older samples do not remain.
"""
...
DEFAULT_TRADES = V2E_ROOT / 'data' / 'mnq_v2e_london_sweep_breaker.csv'
DEFAULT_M1 = MNQ_ROOT / 'raw' / 'glbx-mdp3-20210304-20260303.ohlcv-1m.csv'
OUT_DIR = V2E_ROOT / 'case_studies' / 'temp'


def clean_temp_chart_dir(out_dir: Path) -> None:
    """Remove existing PNGs and INDEX so a new run does not leave stale chart files."""
...
def stratified_month_sample(df: pd.DataFrame, n: int, rng: np.random.Generator) -> pd.DataFrame:
    """Round-robin across year-month keys until n rows (then fill from remainder)."""
    if df.empty or n <= 0:
        return df.iloc[0:0]
    work = df.copy()
...
def draw_one(
    ...
    # London session shading (chart window starts 02:00)
    if len(bars5.index):
        t0 = bars5.index[0]
    ...
    ax.axhline(float(row['breaker_high']), color='#FFD54F', linestyle='-', linewidth=1.1, label='Breaker H / limit')
    ax.axhline(float(row['breaker_low']), color='#FFB74D', linestyle=':', linewidth=0.9, alpha=0.8)
    ...
    title = (
        f'{tag.upper()}  {session_d}  |  Net ${net:+.2f}  |  {row["result"]}  '
...
def main() -> int:
    ...
    clean_temp_chart_dir(args.out)
    index_lines: list[str] = ['# London sweep breaker — temp charts', '', '| File | Net USD | Result |', '|------|--------:|--------|']

    def run_batch(sub_df: pd.DataFrame, prefix: str) -> None:
        for i, (_, row) in enumerate(sub_df.iterrows(), start=1):
            ...
            setup = find_setup_long(session_d, float(ldn_l), day_b)
            if setup is None:
            ...
            out_p = args.out / fname
            draw_one(bars5, session_d, row, setup, ts_order, ldn_h, ldn_l, out_p, prefix)
            index_lines.append(f'| `{fname}` | {float(row["net_usd"]):+.2f} | {row["result"]} |')
            print(fname)
