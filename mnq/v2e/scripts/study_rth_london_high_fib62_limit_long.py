#!/usr/bin/env python3
"""
Causal 1 m study — **RTH London-high touch → 61.8% pullback limit long**.

**London levels:** ``low`` / ``high`` over **[02:00, 09:30)** ET (fixed before RTH).

**Setup (sequential, no lookahead beyond closed bars):**

1. First **RTH** **[09:30, 16:00)** touch of **London high** (``high >= H - eps``) arms the pullback.
2. **Limit buy** at **61.8% Fib retracement** of the London range measured **from H toward L**:
   ``entry = H - phi^{-1} * (H - L)`` with default ``phi^{-1} = (sqrt(5)-1)/2``.
3. While waiting for fill: if **London low** prints (**``low <= L + eps``**) before the limit fills → **no trade**
   (pullback swept the SL reference first).
4. Fill when ``low <= entry`` on a subsequent bar; fill price = ``entry``.
5. After fill: **SL** = **London low**, **TP** = **London high**. Intrabar ordering is **pessimistic**:
   stop before target (same convention as ``rules.v2e_causal._exit_hit`` for longs).
6. If neither SL nor TP by end of RTH, exit at **last RTH 1 m close** before **16:00** (``_eod_exit``).

**PnL:** MNQ **\$2**/point, **\$1.50** RT fee (same as v2e constants).

Example::

  cd potions/mnq/v2e/scripts
  python3 study_rth_london_high_fib62_limit_long.py
"""
from __future__ import annotations

import argparse
import math
import sys
from dataclasses import dataclass, field
from datetime import date
from pathlib import Path
from typing import Literal

import pandas as pd

V2E_SCRIPTS = Path(__file__).resolve().parent
V2E_ROOT = V2E_SCRIPTS.parent
MNQ_ROOT = V2E_ROOT.parent
POTIONS_SCRIPTS = MNQ_ROOT.parent / 'scripts'

sys.path[:0] = [str(V2E_SCRIPTS), str(MNQ_ROOT), str(POTIONS_SCRIPTS)]

from rules.v2e_causal import (  # noqa: E402
    FEE_RT,
    MULT,
    RTH_HI,
    RTH_LO,
    _EPS,
    _eod_exit,
    _exit_hit,
    _net_usd,
    london_low_high,
)

from v2e_causal_live_sim import load_by_day, scan_date_range  # noqa: E402


PHI_INV = (math.sqrt(5.0) - 1.0) / 2.0  # ≈ 0.6180339887498949


def touch_low(lo: float, L: float) -> bool:
    return lo <= L + _EPS


def touch_high(hi: float, H: float) -> bool:
    return hi >= H - _EPS


def limit_buy_hit(low: float, entry: float) -> bool:
    return low <= entry + _EPS


@dataclass
class SessionOutcome:
    session_day: date
    status: Literal[
        'no_london_box',
        'no_rth_high_touch',
        'aborted_ldn_low_before_fill',
        'no_fill_eod',
        'filled',
    ]
    london_low: float = float('nan')
    london_high: float = float('nan')
    entry_px: float = float('nan')
    exit_px: float = float('nan')
    result: str = ''
    net_usd: float = float('nan')
    mae_pts: float = float('nan')
    mfe_pts: float = float('nan')
    ts_first_rth_high: pd.Timestamp = field(default_factory=lambda: pd.NaT)
    ts_fill: pd.Timestamp = field(default_factory=lambda: pd.NaT)
    ts_exit: pd.Timestamp = field(default_factory=lambda: pd.NaT)


def simulate_session(
    day_1m: pd.DataFrame,
    session_day: date,
    *,
    fib_ratio: float,
) -> SessionOutcome:
    L, H = london_low_high(day_1m, session_day)
    if math.isnan(L) or math.isnan(H) or H <= L + _EPS:
        return SessionOutcome(session_day, 'no_london_box')

    entry_limit = H - fib_ratio * (H - L)

    phase: Literal['wait_high', 'wait_fill', 'in_trade'] = 'wait_high'

    day = day_1m.sort_index()
    order_entry = float('nan')
    mae = 0.0
    mfe = 0.0
    first_high_ts = pd.NaT
    fill_ts = pd.NaT

    rth_rows: list[tuple[pd.Timestamp, float, float, float]] = []
    for ts, row in day.iterrows():
        ts = pd.Timestamp(ts)
        if ts.date() != session_day:
            continue
        if not (RTH_LO <= ts.time() < RTH_HI):
            continue
        hi = float(row['high'])
        lo = float(row['low'])
        cl = float(row['close'])
        rth_rows.append((ts, hi, lo, cl))

    if not rth_rows:
        return SessionOutcome(session_day, 'no_rth_high_touch', london_low=L, london_high=H)

    for ts, hi, lo, _cl in rth_rows:
        if phase == 'wait_high':
            if touch_high(hi, H):
                first_high_ts = ts
                phase = 'wait_fill'
                # same-bar: optional pullback / invalidate / fill
                if touch_low(lo, L):
                    return SessionOutcome(
                        session_day,
                        'aborted_ldn_low_before_fill',
                        london_low=L,
                        london_high=H,
                        ts_first_rth_high=first_high_ts,
                    )
                if limit_buy_hit(lo, entry_limit):
                    order_entry = entry_limit
                    phase = 'in_trade'
                    fill_ts = ts
                    ex_px, result, b_mae, b_mfe = _exit_hit(
                        side='long',
                        high=hi,
                        low=lo,
                        entry=order_entry,
                        stop_px=L,
                        tp_px=H,
                    )
                    mae = max(mae, b_mae)
                    mfe = max(mfe, b_mfe)
                    if ex_px is not None and result is not None:
                        nu = _net_usd('long', order_entry, float(ex_px))
                        return SessionOutcome(
                            session_day,
                            'filled',
                            london_low=L,
                            london_high=H,
                            entry_px=order_entry,
                            exit_px=float(ex_px),
                            result=result,
                            net_usd=nu,
                            mae_pts=mae,
                            mfe_pts=mfe,
                            ts_first_rth_high=first_high_ts,
                            ts_fill=fill_ts,
                            ts_exit=ts,
                        )
            continue

        if phase == 'wait_fill':
            if touch_low(lo, L):
                return SessionOutcome(
                    session_day,
                    'aborted_ldn_low_before_fill',
                    london_low=L,
                    london_high=H,
                    ts_first_rth_high=first_high_ts,
                )
            if limit_buy_hit(lo, entry_limit):
                order_entry = entry_limit
                phase = 'in_trade'
                fill_ts = ts
                ex_px, result, b_mae, b_mfe = _exit_hit(
                    side='long',
                    high=hi,
                    low=lo,
                    entry=order_entry,
                    stop_px=L,
                    tp_px=H,
                )
                mae = max(mae, b_mae)
                mfe = max(mfe, b_mfe)
                if ex_px is not None and result is not None:
                    nu = _net_usd('long', order_entry, float(ex_px))
                    return SessionOutcome(
                        session_day,
                        'filled',
                        london_low=L,
                        london_high=H,
                        entry_px=order_entry,
                        exit_px=float(ex_px),
                        result=result,
                        net_usd=nu,
                        mae_pts=mae,
                        mfe_pts=mfe,
                        ts_first_rth_high=first_high_ts,
                        ts_fill=fill_ts,
                        ts_exit=ts,
                    )
            continue

        # in_trade
        assert phase == 'in_trade'
        ex_px, result, b_mae, b_mfe = _exit_hit(
            side='long',
            high=hi,
            low=lo,
            entry=order_entry,
            stop_px=L,
            tp_px=H,
        )
        mae = max(mae, b_mae)
        mfe = max(mfe, b_mfe)
        if ex_px is not None and result is not None:
            nu = _net_usd('long', order_entry, float(ex_px))
            return SessionOutcome(
                session_day,
                'filled',
                london_low=L,
                london_high=H,
                entry_px=order_entry,
                exit_px=float(ex_px),
                result=result,
                net_usd=nu,
                mae_pts=mae,
                mfe_pts=mfe,
                ts_first_rth_high=first_high_ts,
                ts_fill=fill_ts,
                ts_exit=ts,
            )

    if phase == 'wait_high':
        return SessionOutcome(session_day, 'no_rth_high_touch', london_low=L, london_high=H)

    if phase == 'wait_fill':
        return SessionOutcome(
            session_day,
            'no_fill_eod',
            london_low=L,
            london_high=H,
            ts_first_rth_high=first_high_ts,
        )

    # Still in trade at end of RTH loop — flat at last RTH close
    assert phase == 'in_trade'
    eod_ts, eod_px, result = _eod_exit(day, session_day, 'long', order_entry)
    if not pd.isna(eod_ts):
        last = day.loc[eod_ts]
        mae = max(mae, order_entry - float(last['low']))
        mfe = max(mfe, float(last['high']) - order_entry)
    nu = _net_usd('long', order_entry, float(eod_px))
    exit_ts = eod_ts if not pd.isna(eod_ts) else pd.NaT
    return SessionOutcome(
        session_day,
        'filled',
        london_low=L,
        london_high=H,
        entry_px=order_entry,
        exit_px=float(eod_px),
        result=result,
        net_usd=nu,
        mae_pts=mae,
        mfe_pts=mfe,
        ts_first_rth_high=first_high_ts,
        ts_fill=fill_ts,
        ts_exit=exit_ts,
    )


def main() -> int:
    ap = argparse.ArgumentParser(description=__doc__.split('Example::')[0].strip())
    ap.add_argument('--1m', dest='m1', type=Path, default=MNQ_ROOT / 'raw' / 'glbx-mdp3-20210304-20260303.ohlcv-1m.csv')
    ap.add_argument('--start', type=str, default=None)
    ap.add_argument('--end', type=str, default=None)
    ap.add_argument('--fib', type=float, default=PHI_INV, help='Retracement ratio from H toward L (default: φ⁻¹≈0.618)')
    args = ap.parse_args()

    if not args.m1.is_file():
        print(f'Missing 1m CSV {args.m1}', file=sys.stderr)
        return 1

    fib_ratio = float(args.fib)
    if not (0.0 < fib_ratio < 1.0):
        print('--fib must be in (0,1)', file=sys.stderr)
        return 1

    if args.start and args.end:
        date_min = pd.Timestamp(args.start).date()
        date_max = pd.Timestamp(args.end).date()
    else:
        date_min, date_max = scan_date_range(args.m1, args.start, args.end)

    by_day = load_by_day(args.m1, date_min, date_max)

    outcomes: list[SessionOutcome] = []
    for session_day in sorted(by_day.keys()):
        if session_day.weekday() >= 5:
            continue
        day_b = by_day[session_day]
        if day_b.empty:
            continue
        outcomes.append(simulate_session(day_b, session_day, fib_ratio=fib_ratio))

    n_eligible = sum(1 for o in outcomes if o.status != 'no_london_box')
    n_no_box = sum(1 for o in outcomes if o.status == 'no_london_box')
    n_high = sum(1 for o in outcomes if o.status not in {'no_london_box', 'no_rth_high_touch'})
    n_no_high = sum(1 for o in outcomes if o.status == 'no_rth_high_touch')
    n_abort = sum(1 for o in outcomes if o.status == 'aborted_ldn_low_before_fill')
    n_no_fill = sum(1 for o in outcomes if o.status == 'no_fill_eod')
    filled = [o for o in outcomes if o.status == 'filled']

    print('=== RTH London-high → Fib limit long (causal 1 m) ===')
    print(f'Date range: {date_min} .. {date_max}')
    print(f'Fib retracement from H toward L: {fib_ratio:.6f}')
    print(f'Weekday sessions scanned: {len(outcomes)}')
    print(f'  Valid London box [02:00,09:30): {n_eligible}')
    print(f'  Missing London box: {n_no_box}')
    print(f'  RTH never touched London high: {n_no_high}')
    print(f'  After high touch: London low before fill (no trade): {n_abort}')
    print(f'  After high touch: limit never filled by 16:00: {n_no_fill}')
    print(f'  Filled trades: {len(filled)}')

    if filled:
        nets = pd.Series([float(o.net_usd) for o in filled])
        wins = sum(1 for o in filled if str(o.result) == 'Win')
        losses = sum(1 for o in filled if str(o.result) == 'Loss')
        eod_w = sum(1 for o in filled if str(o.result) == 'EOD-Win')
        eod_l = sum(1 for o in filled if str(o.result) == 'EOD-Loss')
        eod_f = sum(1 for o in filled if str(o.result) == 'EOD-Flat')
        be = sum(1 for o in filled if str(o.result) == 'Stop-BE')

        print('\n--- Filled trade outcomes ---')
        print(f'  TP at London high (Win): {wins}')
        print(f'  SL at London low (Loss): {losses}')
        print(f'  Stop-BE: {be}')
        print(f'  EOD exit — Win / Loss / Flat: {eod_w} / {eod_l} / {eod_f}')
        print(f'  Win rate (strict TP only): {100.0 * wins / len(filled):.2f}%')
        print(f'  Win rate (TP + EOD-Win): {100.0 * (wins + eod_w) / len(filled):.2f}%')
        print(f'  Mean net $/trade: {nets.mean():.2f}')
        print(f'  Sum net $: {nets.sum():.2f}')
        eq = nets.cumsum()
        max_dd = float((eq - eq.cummax()).min())
        print(f'  Max drawdown $ (filled trades): {max_dd:.2f}')
        print(f'  Mean MAE pts: {pd.Series([o.mae_pts for o in filled]).mean():.3f}')
        print(f'  Mean MFE pts: {pd.Series([o.mfe_pts for o in filled]).mean():.3f}')

    if n_eligible:
        print('\n--- Rates vs eligible sessions ---')
        print(f'  Any armed path (high touched): {100.0 * (n_eligible - n_no_high) / n_eligible:.2f}%')
        print(f'  Filled / eligible: {100.0 * len(filled) / n_eligible:.2f}%')

    return 0


if __name__ == '__main__':
    raise SystemExit(main())
