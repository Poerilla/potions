#!/usr/bin/env python3
"""Yearly charts for QQQ weekly-pivot market-structure signals."""
...
from qqq_market_structure_weekly_pivot_dca_study import OUT_DIR, display_pattern
...
def load_chart_tables(out_dir: Path, pattern: str, pivot_bars: int, variant: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    weekly_path = out_dir / "weekly_bars.csv"
    signals_path = out_dir / "signals.csv"
    pivots_path = out_dir / "pivots.csv"
    curves_path = out_dir / "curves.csv"
    missing = [path for path in [weekly_path, signals_path, pivots_path, curves_path] if not path.exists()]
    if missing:
        raise FileNotFoundError("Missing weekly study output files: %s" % ", ".join(str(path) for path in missing))

    weekly = pd.read_csv(weekly_path, parse_dates=["date"]).sort_values("date").reset_index(drop=True)
    ...
    if signals.empty:
        raise RuntimeError("No signals found for %s / %d-week pivots." % (pattern, pivot_bars))
    if curves.empty:
        raise RuntimeError("No curve rows found for %s / %d-week pivots / %s." % (pattern, pivot_bars, variant))
    signals["buy_year"] = signals["buy_date"].dt.year
    pivots["pivot_year"] = pivots["pivot_date"].dt.year
    return weekly, signals, pivots, curves


def bounds_for_year(year: int, weekly: pd.DataFrame, year_signals: pd.DataFrame) -> tuple[pd.Timestamp, pd.Timestamp]:
    ...
    end = max(dates) + pd.Timedelta(days=14)
    start = max(start, pd.Timestamp(weekly["date"].min()))
    end = min(end, pd.Timestamp(weekly["date"].max()))
    return start, end
...
def plot_year(
    ...
    start, end = bounds_for_year(year, weekly, year_signals)
    chart_weekly = weekly[(weekly["date"] >= start) & (weekly["date"] <= end)].copy()
    if chart_weekly.empty:
        return {}
    ...
    plot_candles(ax, chart_weekly, width_days=4.3)
    ...
    ax.scatter(
        ...
        linewidths=0.5,
        s=54,
        label="%d-week pivot low" % pivot_bars,
        zorder=6,
    )
    ax.scatter(
        highs["pivot_date"],
        ...
        linewidths=0.5,
        s=54,
        label="%d-week pivot high" % pivot_bars,
        zorder=6,
    )
    ...
    for _, signal in year_signals.iterrows():
        ...
        ax.plot(seq_dates, seq_vals, color="#7c3aed", linewidth=1.45, alpha=0.78, label=sequence_label, zorder=7)
        sequence_label = None
        ax.scatter(signal["l1_pivot_date"], signal["l1_value"], marker="o", color="#0f766e", edgecolors="white", linewidths=0.9, s=95, zorder=8)
        ax.scatter(signal["h1_pivot_date"], signal["h1_value"], marker="o", color="#b45309", edgecolors="white", linewidths=0.9, s=95, zorder=8)
        ax.scatter(signal["l2_pivot_date"], signal["l2_value"], marker="o", color="#dc2626", edgecolors="white", linewidths=0.9, s=110, zorder=8)
    ...
    volume_colors = ["#168a5a" if c >= o else "#c43d3d" for o, c in zip(chart_weekly["open"], chart_weekly["close"])]
    ax_vol.bar(chart_weekly["date"], chart_weekly["volume"] / 1_000_000.0, color=volume_colors, width=4.2, alpha=0.42)
    ax_vol.set_ylabel("Vol (M)")
    ax_vol.grid(True, axis="y", color="#e5e7eb", linewidth=0.6, alpha=0.8)

    ax.set_title(
        "QQQ weekly %s signal, %d-week pivots - %d (%d signals, %d Dec sweeps)"
...
def write_index(chart_dir: Path, rows: list[dict], pattern: str, pivot_bars: int, variant: str, weekly: pd.DataFrame) -> None:
    total_signals = sum(row["signals"] for row in rows)
    total_sweeps = sum(row["dec_sweeps"] for row in rows)
    lines = [
        "# QQQ Weekly %s Yearly Charts" % display_pattern(pattern),
        "",
        "These charts visualize the **%d-week confirmed pivot** version of the weekly-pivot market-structure DCA study." % pivot_bars,
        "",
        "Variant shown: **`%s`**." % variant,
        "",
        "Window: **%s through %s**." % (weekly["date"].min().date().isoformat(), weekly["date"].max().date().isoformat()),
        "",
        "Markers:",
        "",
        "- Green triangles: confirmed weekly pivot lows.",
        "- Orange triangles: confirmed weekly pivot highs.",
        "- Purple line: weekly low -> high -> lower-low sequence.",
        "- Black/yellow star: next available daily open after the weekly signal is known.",
        ...
        "",
        "Total %d-week %s buy signals: **%d**. December catch-up buys: **%d**." % (pivot_bars, display_pattern(pattern), total_signals, total_sweeps),
        "",
...
def main() -> None:
    ...
    weekly, signals, pivots, curves = load_chart_tables(args.output_root, args.pattern, args.pivot_bars, args.variant)
    chart_dir = args.chart_dir or args.output_root / "charts" / ("yearly_%s_%dw" % (display_pattern(args.pattern).lower().replace("-", ""), args.pivot_bars))
    chart_dir.mkdir(parents=True, exist_ok=True)

    years = list(range(int(weekly["date"].dt.year.min()), int(weekly["date"].dt.year.max()) + 1))
    rows = []
    for year in years:
        row = plot_year(year, weekly, signals, pivots, curves, args.pattern, args.pivot_bars, args.variant, chart_dir / ("%d.png" % year))
        if row:
            rows.append(row)
    write_index(chart_dir, rows, args.pattern, args.pivot_bars, args.variant, weekly)
