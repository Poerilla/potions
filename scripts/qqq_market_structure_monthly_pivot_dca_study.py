...
from qqq_market_structure_weekly_pivot_dca_study import (
    display_pattern,
    next_daily_open_after,
    signal_dca_with_year_end_sweep,
    summarize_curve_with_sweeps,
    weekly_ohlcv,
)
from qqq_yearly_orb_study import default_completed_end, load_adjusted_daily


OUT_DIR = ROOT / "nq" / "case_studies" / "qqq_market_structure_monthly_pivot_dca_study"
DEFAULT_START = "2000-01-01"


def monthly_ohlcv(daily: pd.DataFrame, drop_final_partial: bool = True) -> tuple[pd.DataFrame, str]:
    ...
    dropped = ""
    if drop_final_partial and not monthly.empty:
        last_daily = pd.Timestamp(work["date"].max())
        if last_daily.day < 25:
            dropped = pd.Timestamp(monthly.iloc[-1]["date"]).date().isoformat()
            monthly = monthly.iloc[:-1].copy().reset_index(drop=True)
...
def detect_lhll_monthly(daily: pd.DataFrame, monthly: pd.DataFrame, pivot_bars: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    pivots = build_confirmed_pivots(monthly, pivot_bars, pivot_bars)
    l1: Pivot | None = None
    h1: Pivot | None = None
    signals = []
                        ...
                        signals.append(
                            {
                                ...
                                "buy_price": buy_price,
                                "months_l1_to_h1": int(h1.pivot_idx - l1.pivot_idx),
                                "months_h1_to_l2": int(pivot.pivot_idx - h1.pivot_idx),
                                "months_l2_to_signal": 0,
...
def detect_lhllhh_monthly(
    ...
    armed = False
    signals = []
    closes = pd.to_numeric(monthly["close"], errors="coerce").tolist()
    highs = pd.to_numeric(monthly["high"], errors="coerce").tolist()
    dates = pd.to_datetime(monthly["date"]).tolist()
                    ...
                    signals.append(
                        {
                            ...
                            "buy_price": buy_price,
                            "months_l1_to_h1": int(h1.pivot_idx - l1.pivot_idx),
                            "months_h1_to_l2": int(l2.pivot_idx - h1.pivot_idx),
                            "months_l2_to_signal": int(idx - l2.pivot_idx),
...
def run_variant(
    ...
    if pattern == "low_high_lower_low":
        signals, pivots = detect_lhll_monthly(daily, monthly, pivot_bars)
    elif pattern == "low_high_lower_low_higher_high":
        signals, pivots = detect_lhllhh_monthly(daily, monthly, pivot_bars, break_mode)
    else:
...
def plot_top_causal(summary: pd.DataFrame, monthly_summary: dict, out: Path) -> None:
    ...
    causal["label"] = causal["pattern"].str.replace("low_high_", "L-H-", regex=False).str.replace("_", "-") + " / " + causal["pivot_bars"].astype(str) + "m"
    ...
    ax.set_title("Monthly-pivot causal signal DCA vs monthly DCA")
...
def chart_stem(pattern: str, pivot_bars: int) -> str:
    return "yearly_%s_%dm" % (display_pattern(pattern).lower().replace("-", ""), pivot_bars)
...
def write_report(
    ...
    best_chart = chart_stem(str(best_any["pattern"]), int(best_any["pivot_bars"]))

    lines = [
        "# QQQ Monthly-Pivot Market-Structure DCA Study",
        "",
        "Data: Yahoo adjusted daily OHLCV for `QQQ`; monthly candles are resampled from adjusted daily bars.",
        "Window: **%s through %s** (%d monthly bars)." % (daily["date"].min().date().isoformat(), daily["date"].max().date().isoformat(), len(monthly_bars)),
    ]
    if dropped_final_month:
        lines.append("Final partial monthly bar ending **%s** was dropped before pivot detection." % dropped_final_month)
    lines.extend(
        [
            "",
            "## Rule",
            ...
            "- Cashflow comparison: contribute **%s/month**. Monthly DCA buys the first trading day open; signal variants hold cash and buy on monthly-swing signals." % money(monthly_amount),
            "",
            "## Leaderboard",
            "",
    ...
    lines.extend(
        [
            ...
            "## Read",
            "",
            "- Best causal monthly-swing row: **%s / %d-month pivots / %s**, with **%d signals** (%.2f/year), **%d signal buys** and **%d December sweeps**, ending at **%s** (**%s** vs monthly DCA)."
            % (
            ...
            "- Best any-mode row: **%s / %d-month pivots / %s**, with **%d signal buys** and **%d December sweeps**, ending at **%s** (**%s** vs monthly DCA)."
            % (
    ...
    lines.extend(
        [
            ...
            "- Best monthly row yearly chart pack: [`charts/%s/INDEX.md`](charts/%s/INDEX.md)" % (best_chart, best_chart),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run QQQ monthly-pivot market-structure DCA study.")
    ...
    parser.add_argument("--keep-final-partial-month", action="store_true")
    ...
    monthly_bars, dropped_final_month = monthly_ohlcv(daily, drop_final_partial=not args.keep_final_partial_month)
    ...
    monthly_bars.to_csv(out_dir / "monthly_bars.csv", index=False)
    ...
    for pattern, stem in [("low_high_lower_low", "lhll"), ("low_high_lower_low_higher_high", "lhllhh")]:
        ...
        plot_equity_local(
            ...
            "QQQ monthly %s DCA comparison (%d-month pivots)" % (display_pattern(pattern), best_pivot),
        )

    write_report(out_dir, daily, monthly_bars, dropped_final_month, monthly_summary, summary, signals_all, args.monthly_amount)
